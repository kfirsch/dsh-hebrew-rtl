...
(function () {
  var CSS = [
    // `td`/`th` are absent on purpose. Giving a cell `unicode-bidi: plaintext`
    // makes it pick its own direction from its first strong character, which
    // is exactly the per-cell disagreement the table-level rule below exists
    // to prevent: cells must inherit the table's direction, not re-derive one.
    'body :where(p, li, h1, h2, h3, h4, h5, h6, blockquote, span, div):not(input):not(textarea):not([contenteditable]):not([contenteditable] *):not([class*="_NYaKW_"]):not([class*="_NYaKW_"] *) {',
    '  unicode-bidi: plaintext;',
    '}',
    // A table with no Hebrew is left to the page default; one with Hebrew has
    // its direction set by the JS rule. Either way the cells align to whatever
    // direction is in force rather than to a hard-coded edge.
    'table td, table th {',
    '  text-align: start;',
    '}',
  ...
  //
  // `table` rather than `td, th`: a table is ONE directional unit. Judging
  // each cell separately gave a six-row table three different verdicts —
  // a Hebrew label cell went RTL while the `HEAD` and `Commits` cells beside
  // it stayed LTR — so the label column flipped edges from row to row and the
  // eye had no single alignment to follow. Worse, column ORDER is a property
  // of the table, not the cell, so per-cell directions fight the column order
  // itself. Deciding once on the table's whole text and letting every cell
  // inherit keeps column order and alignment consistent; `unicode-bidi:
  // isolate` still lays out each cell's own content correctly inside it.
  var BLOCK_SELECTOR = 'p, li, h1, h2, h3, h4, h5, h6, blockquote, table'
